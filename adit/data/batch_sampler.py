...
# donner au dataloader.
# prend qq minutes au début du run
# batch norm prp atome/residue, pas la protéine

# ajouter bruit sur coords pour introduire stocha
# mais bcp de données avec adit -> batchs déjà assez hétérogène, pas nécessaire
...
class AtomBudgetBatchSampler(Sampler[List[int]]):
    ...
    Args:
        ...
        max_atoms: Maximum total atoms per batch eg 20k
        min_batch_size: Target minimum samples per batch (soft constraint in FFD) (on veut pas de batch de taille 1 -> mis à 2)
        shuffle: Whether to shuffle the order of batches each epoch # seul stocha: ordre des batches changé, mais le contenu des batchs ne change pas
