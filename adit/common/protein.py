...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> Protein:
  ...
  residue_index_hack = 1 # hack: deal with insertion codes in pdb file
  for chain in model:
    ...
    for res in chain:
      ...
      residue_index.append(residue_index_hack) # hack
      residue_index_hack += 1
...
class SkempiProtein:
  ...
  original_index: np.ndarray  # [num_res]
...
def skempi_from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> Protein:
  """Takes a PDB string and constructs a Protein object.
  ...
  for chain in model:
    ...
    for res in chain:
      ...
      # residue_index.append(res.id[1])
      residue_index.append(residue_index_hack) # hack
      original_index.append(res.id[1]) # hack
      residue_index_hack += 1
      chain_ids.append(chain.id)
      b_factors.append(res_b_factors)
...
def from_prediction(
    ...
    pred_atom_positions: np.ndarray,
  ...
  Args:
    ...
    pred_atom_positions: model outputs.
  ...
  if b_factors is None:
    b_factors = np.zeros_like(features['atom_mask'])

  return Protein(
      aatype=_maybe_remove_leading_dim(features['aatype']),
      atom_positions=pred_atom_positions,
